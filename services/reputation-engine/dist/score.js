...
export function calculateReliability(factors) {
    const attempts = Math.max(0, factors.verifiedAttempts);
    const tests = Math.max(0, factors.totalSandboxTests);
    const verifiedSuccessRate = (Math.max(0, factors.verifiedSuccesses) + 2) / (attempts + 4);
    const hallucinationIndex = (Math.max(0, factors.failedSandboxTests) + 1) / (tests + 10);
    const speedRatio = factors.medianRuntimeMs <= 0
        ? 1
    ...
    const weighted = 0.55 * verifiedSuccessRate +
        0.2 * speedScore +
