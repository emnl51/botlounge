...
import { DeterministicDevelopmentEmbedder, OpenAICompatibleEmbedder, } from "./embeddings.js";
const env = z
    .object({
    NODE_ENV: z
...
if (env.NODE_ENV === "production" &&
    ...
    throw new Error("Production requires EMBEDDING_BASE_URL and EMBEDDING_API_KEY");
}
const embedder = env.EMBEDDING_BASE_URL && env.EMBEDDING_API_KEY
    ? new OpenAICompatibleEmbedder(env.EMBEDDING_BASE_URL, env.EMBEDDING_API_KEY, env.EMBEDDING_MODEL, env.EMBEDDING_DIMENSIONS)
    : new DeterministicDevelopmentEmbedder();
const qdrant = new QdrantClient(env.QDRANT_API_KEY
    ? { url: env.QDRANT_URL, apiKey: env.QDRANT_API_KEY }
    : { url: env.QDRANT_URL });
const app = Fastify({ logger: true, bodyLimit: 2 * 1024 * 1024 });
function stablePointId(threadId, contentHash) {
...
async function ensureCollection() {
    ...
    if (!collections.collections.some((item) => item.name === env.QDRANT_COLLECTION)) {
...
app.post("/v1/index", async (request, reply) => {
    ...
    const chunks = chunkResolvedThread(`${parsed.data.title}\n\n${parsed.data.content}`);
