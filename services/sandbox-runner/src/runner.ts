...
export class DockerSandboxRunner {
  constructor(
    ...
    private readonly images: Record<"python" | "javascript", string>,
    private readonly ociRuntime?: string,
  ) {}

  async execute(request: ExecutionRequest): Promise<ExecutionResult> {
    const runtime = this.runtimeConfig(request.runtime);
    const memoryBytes = request.limits.memoryMb * 1024 * 1024;
    const workspaceMb = Math.min(
      32,
      Math.max(
        4,
        Math.ceil(
          (Buffer.byteLength(request.source) +
    ...
    const append = (current: string, chunk: Buffer) =>
      (current + chunk.toString("utf8")).slice(0, request.limits.outputBytes);
    ...
    let statsStream: Readable | undefined;
    const start = performance.now();
    try {
      await container.putArchive(
        await this.sourceArchive(runtime, request.source, request.testSource),
        { path: "/workspace" },
      );
      ...
      statsStream = (await container.stats({ stream: true })) as Readable;
      let statsBuffer = "";
      ...
      const assertions = parseAssertionMetrics(
        request.runtime,
        stdoutText,
        stderrText,
      );
  ...
  private runtimeConfig(runtime: "python" | "javascript"): RuntimeConfig {
    if (runtime === "python") {
      return {
